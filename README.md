...
| `ALERT_EMAIL_TO` | Recipient for push result emails. Optional; unset means notifications are silently skipped |
...
**Push result emails**: every call to `syncCheckinsToFrappe` (all three triggers above) sends an email with the outcome — status, records fetched/created/skipped/failed, and the error if it failed. This fires on every single call, including no-op runs with nothing new to push, by design (see `lib/frappe/checkin.ts`'s `notifySyncResult`). Configured via `SMTP_HOST`/`SMTP_PORT`/`SMTP_USER`/`SMTP_PASSWORD`/`SMTP_FROM`/`ALERT_EMAIL_TO`; silently a no-op if SMTP env vars or `ALERT_EMAIL_TO` aren't set, and a send failure is logged but never breaks the sync it's reporting on (`lib/email/mailer.ts`).
