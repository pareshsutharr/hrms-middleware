...
| `SMTP_HOST` / `SMTP_PORT` / `SMTP_USER` / `SMTP_PASSWORD` / `SMTP_FROM` | Outbound SMTP relay for Frappe push result emails — see [Automatic Frappe push](#automatic-frappe-push). All optional; unset means notifications are silently skipped |
| `ALERT_EMAIL_TO` | Default recipient for push result emails — can be overridden from Settings > Email Alerts in the UI. Optional; unset (and no override saved) means notifications are silently skipped |
...
**Push result emails**: every call to `syncCheckinsToFrappe` (all three triggers above) can send an email with the outcome — status, records fetched/created/skipped/failed, and the error if it failed. Recipient and frequency are configurable from **Settings > Email Alerts** (`components/settings/email-alerts-config-form.tsx`, `lib/email/config.ts`) — same override-over-env pattern as COSEC/Frappe settings, stored in `SystemSetting` under `email.recipientEmail` / `email.frequency`. Frequency options: every push attempt, only when something changed or failed, only on failure, or off. Clearing the recipient field (or setting frequency to "off") disables alerts entirely. SMTP transport itself (`SMTP_HOST`/`SMTP_PORT`/`SMTP_USER`/`SMTP_PASSWORD`/`SMTP_FROM`) is env-var only, not exposed in the UI. A send failure is logged but never breaks the sync it's reporting on (`lib/email/mailer.ts`).
...
Built (Phase 3): `/agent/cosec-agent` and the cloud-side `/api/agent/*` endpoints it relays to (see [The COSEC Agent](#the-cosec-agent)) — fully verified end-to-end in production, not just the heartbeat: running as a persistent macOS LaunchAgent (auto-restart on crash, auto-start on login), it has relayed real COSEC attendance/event data into the production database and, from there, real Employee Checkins into Frappe (see below).
...
Built: automatic Frappe push (agent-triggered + daily Vercel Cron safety net) and the "Open Attendence" shift exclusion — see [Automatic Frappe push](#automatic-frappe-push). **Fully verified end-to-end in production, not just individually**: production's `employee_mappings` is populated (16 employees, confirmed correct name-by-name against the live Frappe roster), and a real sync pushed 14 real Employee Checkins into Frappe for real COSEC punches that day (confirmed by reading them back directly from Frappe). Checkout (`log_type: OUT`) uses the identical code path as check-in and is deployed, but hasn't been exercised by a real OUT-type punch yet — nothing to fix, just nothing to prove against so far.

Not built yet, by design (see the original project spec for full detail):
- Nothing currently blocked on missing functionality — remaining gaps are data/business-side, not code: `HARSHITA SHEWANI` (COSEC `JBV0026`) has no Frappe Employee record yet, so she can't be mapped until she's added there; a handful of Frappe employees intentionally have no COSEC device mapping (company-granted flexibility on punching, not a gap — see [Employee mapping](#employee-mapping)).

## Operational notes

- **Supabase free plan**: the production database is on Supabase's free tier, which auto-pauses a project after 7 days with no database activity. In practice the agent's ~5-minute heartbeat/sync cycle and the daily cron keep it well within that window, but if the machine running the agent is ever offline for a week+ with nobody visiting the dashboard either, the project would pause and the whole app would go down until manually resumed from the Supabase dashboard (no data loss, just downtime). Upgrading to Supabase Pro removes this entirely.
- **The Agent's persistence** is a macOS LaunchAgent (`~/Library/LaunchAgents/com.pareshsuthar.cosec-agent.plist` on the machine running it) — starts on login, restarts on crash. It fundamentally requires a machine physically on the COSEC LAN to stay powered on and connected; that's inherent to Mode B, not a software gap.
